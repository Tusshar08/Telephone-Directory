...
public class Directory {
    ...
    private String fail = ""; // Output failure reason
    ...
    private boolean validPhone(String phone){
        ...
        if(phone ==  null || phone.length() !=7){
            fail = "Invalid Number Length";
            return false;
        }

        for(char ch : phone.toCharArray()){
            if(!Character.isDigit(ch)){
                fail = "Invalid Number: Has Other Characters";
                return false;
            }
        }

        if(phone.charAt(0) == '0'){
            fail = "Invalid First Digit";
            return false;
        }
        ...
        for (int i = 0; i < TABLES; i++) {
            ...
            for (int j = 0; j < TABLE_SIZE; j++) {
                ...
                if (tables[i][j].status == Status.EMPTY) {
                    ...
                    out.println( "Name: " + tables[i][j].name + ", Phone: " + tables[i][j].phone + ", Address: " + tables[i][j].address );
                }
            }
        }
    }
        ...
        while(sc.hasNextLine()) {
            ...
            switch(section[0].toLowerCase()) {
                ...
                case "insert":{
                    ...
                    out.println(inserted ? "Success" : "Failure: " + directory.getFail());

                    break;
                }
                case "delete":{
                    boolean delete = directory.delete(section[1]);
                    out.println(delete ? "Success" : "Failure: " + directory.getFail());
                ...
                case "modify":{
                    ...
                    out.println(modified ? "Success" : "Failure: " + directory.getFail());

                    break;
                }

                case "print":
                    directory.display(out);
